"""
Correctness tests for onednn_int4_gemm_torchao (asymmetric torchao INT4).

Regression-protects the format contract verified by Blackwood416 on A770:
...
def test_torchao_gemm_matches_reference(xpu_device, M, K, N, gs, dtype):
    svdq = _native_svdq()
    torch.manual_seed(7)
    ...
    out = svdq.onednn_int4_gemm_torchao(act, packed, zp, scale)
    ref = ref_forward(act, q_nk, zp, scale, gs)

    assert out.shape == ref.shape
    assert torch.isfinite(out).all()
    # oneDNN fp16/bf16 accumulation noise; f32 tiny shape is near-exact.
    ...
    assert torch.allclose(out.to(torch.float32), ref, atol=atol, rtol=rtol), (
        f"torchao GEMM mismatch: max abs {torch.abs(out - ref).max().item():.3e}")


def test_torchao_gemm_qwen_scale_k_no_nan(xpu_device):
...
def test_torchao_gemm_rejects_cpu_tensors(xpu_device):
    ...
    with pytest.raises(RuntimeError, match="XPU"):
        svdq.onednn_int4_gemm_torchao(act, packed, zp, scale)

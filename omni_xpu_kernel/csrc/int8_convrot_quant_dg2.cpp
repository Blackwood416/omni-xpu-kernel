...
//             writes the group absmax to a small [rows, groups] scratch;
...
//
// The two-pass recompute avoids holding a whole row in SLM (the row-SLM
// variant was measured 0.4-0.8x slower than the composed path on A770).
// Total traffic is one activation read per kernel plus the int8 write,
// instead of rotate-write + rotate-read + quantize-read + int8-write.
//
...
// Negative result recorded 2026-08-15 (A770, driver 32.0.101.8860): the
// first per-group variants early-returned invalid subgroups before
// item.barrier(), a divergent work-group barrier that triggered
// UR_RESULT_ERROR_DEVICE_LOST on K=7168/256. This variant never returns
// before a barrier; invalid subgroups are clamped to a valid group and mask
// their stores. A plain scratch + reduce kernel is used instead of atomics.
...
void rotate_group_in_slm(
    ...
    const sycl::local_accessor<float, 1>& slm,
    sycl::nd_item<1> item) {
    // Lane loads 8 consecutive bf16/f16 elements of this group.
    constexpr int VEC = 8;
    ...
    if (group_size % VEC == 0) {
        ...
        for (int64_t k = lane_start; k < group_size;
            ...
            const VecT values = *reinterpret_cast<const VecT*>(
                row_ptr + group_offset + k);
            for (int i = 0; i < VEC; ++i) {
                slm[slm_offset + k + i] = static_cast<float>(values[i]);
            }
        }
    } else {
        for (int64_t k = lane; k < group_size; k += DG2_CONVROT_SG) {
            slm[slm_offset + k] = static_cast<float>(row_ptr[group_offset + k]);
        }
    }
    item.barrier(sycl::access::fence_space::local_space);

    // Radix-4 Hadamard butterfly. One butterfly covers the 4 elements that
    // share every digit except the current one; iterate all positions and
    // keep those whose current digit is zero.
    for (int64_t stage = 0; stage < stages; ++stage) {
        const int64_t stride = static_cast<int64_t>(1) << (2 * stage);
        for (int64_t p = lane; p < group_size; p += DG2_CONVROT_SG) {
            if (((p / stride) % 4) != 0) continue;
            const float x0 = slm[slm_offset + p];
            const float x1 = slm[slm_offset + p + stride];
            const float x2 = slm[slm_offset + p + 2 * stride];
            const float x3 = slm[slm_offset + p + 3 * stride];
            slm[slm_offset + p] = x0 + x1 + x2 - x3;
            slm[slm_offset + p + stride] = x0 + x1 - x2 + x3;
            slm[slm_offset + p + 2 * stride] = x0 - x1 + x2 + x3;
            slm[slm_offset + p + 3 * stride] = -x0 + x1 + x2 + x3;
        }
        item.barrier(sycl::access::fence_space::local_space);
    }

    // Materialize the rotated value in the input dtype exactly like the
    // composed matmul path; this preserves its bf16 rounding boundary.
    for (int64_t k = lane; k < group_size; k += DG2_CONVROT_SG) {
        slm[slm_offset + k] = static_cast<float>(
            static_cast<InputT>(slm[slm_offset + k] * inv_sqrt_group));
    }
    item.barrier(sycl::access::fence_space::local_space);
}
    ...
    auto cgf = [&](sycl::handler& handle) {
        ...
        handle.parallel_for(
            ...
            [=](sycl::nd_item<1> item)
                ...
                // Never return before a work-group barrier: pad invalid
                // subgroups to a valid group and mask their stores instead.
                // Early-returning from a subgroup while others execute
                // item.barrier() is undefined and produced DEVICE_LOST on
                // A770 for K=7168 (28 groups) and K=256 (1 group).
                const int64_t group =
                    raw_group < groups ? raw_group : (groups - 1);
                const bool valid = raw_group < groups;

                const int64_t group_offset = group * group_size;
                rotate_group_in_slm<InputT>(
                    input + row * groups * group_size, group_offset,
                    sub * group_size, lane,
                    group_size, stages, inv_sqrt_group, slm, item);

                // Each lane covers group_size/WG elements.
                float local_max = 0.0f;
                for (int64_t k = lane; k < group_size; k += DG2_CONVROT_SG) {
                    local_max = sycl::fmax(
                        local_max,
                        sycl::fabs(slm[sub * group_size + k]));
                }
...
void convrot_row_reduce_kernel(
    ...
    int64_t groups,
    const at::Device& device) {
    // One subgroup (32 lanes) per row; no SLM, no atomics.
    ...
    auto cgf = [&](sycl::handler& handle) {
        handle.parallel_for(
            ...
            [=](sycl::nd_item<1> item)
                ...
                if (lane == 0) {
                    const float scale =
                        row_max > 0.0f ? row_max / 127.0f : 0.0f;
            ...
            [=](sycl::nd_item<1> item)
                ...
                if (valid) {
                    for (int64_t k = lane; k < group_size;
                         k += DG2_CONVROT_SG) {
                        float q = sycl::rint(
                            slm[sub * group_size + k] * quant_inv);
...
void dispatch_dg2(
    ...
    convrot_row_reduce_kernel<InputT>(
        group_maxes_ptr, scales_ptr, rows, groups, input.device());

// Parameterized FP16-IO Flash Attention kernel -- fp16 input/output, fp32 S*V accumulation
// DG2 (Xe-HPG) variant: the Xe2 kernels use an fp16 S*V accumulator
// (dpas.hf.hf), which the DG2 VISA validator rejects. This variant keeps
// fp16 Q/K/V I/O but accumulates S*V in fp32 (dpas<8,8,float,float,fp16,fp16>),
// which is valid on both Xe-HPG and Xe2.
//   Barrier moved before compensation (SLM load + ALU overlap).
//   Interleaved by l-group for S*V DPAS overlap.
//
// QK DPAS: dpas<8,8,float,float,fp16,fp16> (fp16 inputs, fp32 accum for softmax precision)
// S*V DPAS: dpas<8,8,float,float,fp16,fp16> (fp16 inputs, fp32 accum -- no clamp needed)
//
// Supports HEAD_DIM=64 and HEAD_DIM=128 via Cfg template from sdp_config.h.


ESIMD_INLINE void flashAttnBMhaFp16Fp32AccumPrecomputed(
  uint8_t* qState,
  uint8_t* kState,
  uint8_t* vState,
  uint8_t* normAlpha,
  ...
  // Q stored as fp16 (no conversion needed -- used directly in fp16 QK DPAS)
  ...
  // S*V accumulator is fp32 (no clamp needed)
  simd<float, 16 * HD> finalOutput = 0;
  ...
  // Load first V block (fp16 bits loaded as fp16, only active V column threads)
  if (hhv < V_COL_GROUPS) {
  ...
  // Load Q as fp16 (no conversion -- used directly in fp16 QK DPAS)
  {
  ...
  for (loopIdx = 0; loopIdx < kvSeqOutLoopCount - 1; loopIdx++) {
    ...
    simd<float, 32> fp32CompTemp;      // fp32 compensation for accumulator multiply
    tempOutput = 0;

    // ===== Q @ K^T (fp16 DPAS -- no conversion needed for Q or K) =====
    {
      #pragma unroll
    ...
    // ===== V load + fp16 conversion (only active V column threads) =====
    if (hhv < V_COL_GROUPS) {
      fp16VState =
        __ESIMD_ENS::lsc_load_2d<fp16, 16, 16, 2, false, true,
        __ESIMD_ENS::cache_hint::cached, __ESIMD_ENS::cache_hint::cached>(payloadV);
    ...
    // ===== Softmax =====
    {
      ...
      // Broadcast compensation to fp32 for accumulator multiply after barrier
      fp32CompTemp.select<16, 1>(0) = fp32SoftMaxCompensation;
      fp32CompTemp.select<16, 1>(16) = fp32SoftMaxCompensation;
    ...
    // ===== Interleaved compensation + S*V by output-dim half =====
    {
      ...
      for (int kk = 0; kk < HD_QUARTER; kk++) {
        finalOutput.select<32, 1>(32 * kk) = finalOutput.select<32, 1>(32 * kk) * fp32CompTemp;
      }
  ...
  // Output normalization -- fp32 accumulator, convert to fp16 at the end
  ...
  for (int kk = 0; kk < HD_HALF; kk++) {
    ...
    simd<float, 32> f32Temp = finalOutput.select<32, 1>(32 * kk);
    ...
    f32Temp = f32Temp * alphaMul;
    f32Temp.merge(65504.0f, f32Temp > 65504.0f);
    f32Temp.merge(-65504.0f, f32Temp < -65504.0f);
    // Convert fp32 -> fp16 directly for output
    fp16QState.select<16, 2>(32 * kk) = f32Temp.select<16, 1>(0);
    fp16QState.select<16, 2>(32 * kk + 1) = f32Temp.select<16, 1>(16);
  }
